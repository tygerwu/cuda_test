...
CudaSGemmMicroBufferImplV2(const float *__restrict__ A,
...
template <size_t MC, size_t KC, size_t NC, size_t MR, size_t NR, size_t WY,
          size_t WX>
static void CudaSGemmMicroBufferV2(const float *A, const float *B, float *C,
                                   int M, int N, int K) {
  dim3 dimBlock(NC / NR, MC / MR);
  dim3 dimGrid(N / NC, M / MC);
  CudaSGemmMicroBufferImplV2<MC, KC, NC, MR, NR, WY, WX>
